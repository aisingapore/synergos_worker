...
from rest_rpc.core.utils import MetaRecords
...
###########################
# MetaRecords Class Tests #
###########################

def test_MetaRecords_create():
    meta_records = MetaRecords(db_path=test_db_path)
    created_metadata = meta_records.create(
...
def test_MetaRecords_read_all():
    meta_records = MetaRecords(db_path=test_db_path)
    all_metadata = meta_records.read_all()
...
def test_MetaRecords_read():
    meta_records = MetaRecords(db_path=test_db_path)
    retrieved_metadata = meta_records.read(project_id=project_id)
...
def test_MetaRecords_update():
    meta_records = MetaRecords(db_path=test_db_path)
    targeted_metadata_1 = meta_records.read(project_id=project_id)
    updated_metadata_1 = meta_records.update(
        project_id=project_id,
    ...
    targeted_metadata_2 = meta_records.read(project_id=project_id)
    updated_metadata_2 = meta_records.update(
        project_id=project_id,
...
def test_MetaRecords_delete():
    meta_records = MetaRecords(db_path=test_db_path)
    targeted_metadata = meta_records.read(project_id=project_id)
    deleted_metadata = meta_records.delete(project_id=project_id)
    assert targeted_metadata.doc_id == deleted_metadata.doc_id
    assert meta_records.read(project_id=project_id) is None
